                    ...
                    using (cnLocal = new SqlConnection(ThisServer))
                    {
                        ...
                        if (location == Locations.Kjqtl || location == Locations.Kjsaw)
                            sql = "select CompCode, EmpUnqId, WrkGrp, EmpName, FatherName, " +
                                  "Active, UnitCode, DeptCode, StatCode, CatCode, " +
                                  "DesgCode, GradeCode, 0 as IsHod, 0 as IsReleaser, 0 as IsHrUser, OtFlg as OtFlag, " +
                        ...
                        sql = "merge into Employees as target " +
                              ...
                              "when not matched then " +
                              "insert (empunqid, compcode, wrkgrp, " +
                              "unitcode, deptcode, statcode, " +
                              //"seccode, " +
                              "catcode, " +
                              "desgcode, gradecode, empname, fathername, " +
                              "active, OtFlag, ishod, isreleaser, ishruser, pass, " +
                              "isadmin, isgpreleaser, isgareleaser, issecuser, location, companyacc, " +
                              "birthdate, joindate ) " +
                              "values (source.empunqid, source.compcode, source.wrkgrp, " +
                              ...
                              "'" + location + "', 0, source.BirthDate, source.JoinDate ); ";
