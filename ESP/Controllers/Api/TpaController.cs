...
using ESP.Dto;
using ESP.Models;
using ESP.Migrations;
using Newtonsoft.Json;
using TpaSanction = ESP.Models.TpaSanction;
...
namespace ESP.Controllers.Api
{
    public class TpaController : ApiController
    {
        ...
        public IHttpActionResult GetTpa(DateTime fromDate, DateTime toDate, string empUnqId)
        {
            ...
            List<Models.TpaSanction> alreadyTpaEntries = _context.TpaSanctions.Where(
            ...
            foreach (string emp in emps)
            {
                ...
                for (DateTime dt = fromDate; dt <= toDate;)
                {
                    ...
                    Models.TpaSanction already = alreadyTpaEntries.FirstOrDefault(t => t.TpaDate == dt &&
                        t.EmpUnqId == emp);
                    if (already != null)
                    {
        ...
        public IHttpActionResult PreTpa(string empUnqId, [FromBody] object requestData)
        {
            ...
            try
            {
                ...
                using (DbContextTransaction transaction = _context.Database.BeginTransaction())
                {
                    ...
                    {
                        var tpaSanction = new Models.TpaSanction
                        {
                            Id = 0,
        ...
        public IHttpActionResult GetPreRelease(string empUnqId)
        {
            ...
            List<TpaSanctionDto> tpaSanctionList = _context.TpaSanctions
                ...
                .Select(Mapper.Map<Models.TpaSanction, TpaSanctionDto>)
                .ToList();
            ...
            try
            {
                ...
                using (DbContextTransaction transaction = _context.Database.BeginTransaction())
                {
                    foreach (TpaReleaseDto dto in tpaRleaseDtos)
                    {
                        ...
                        Models.TpaSanction tpaSanction = _context.TpaSanctions
                ...
                using (DbContextTransaction transaction = _context.Database.BeginTransaction())
                {
                    foreach (TpaSanctionDto dto in tpaSanctionDto)
                    {
                        ...
                        if (tpaSanction == null)
                        {
                            ...
                            tpaSanction = new Models.TpaSanction
                            {
                                Id = 0,
                                EmpUnqId = dto.EmpUnqId,
                                TpaDate = dto.TpaDate,
        ...
        public IHttpActionResult PostHrRelease([FromBody] object requestData)
        {
            try
            {
                ...
                using (DbContextTransaction transaction = _context.Database.BeginTransaction())
                {
                    List<Models.TpaSanction> tpaSanctions = _context.TpaSanctions.Where(t => ids.Contains(t.Id)).ToList();

                    foreach (Models.TpaSanction sanction in tpaSanctions)
                    {
